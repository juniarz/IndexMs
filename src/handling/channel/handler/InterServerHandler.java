...
public class InterServerHandler {
	...
	public static final void Loggedin(final int playerid, final MapleClient c) {
		...
		/*
		 * if (player.isGM()) { SkillFactory.getSkill(GameConstants.GMS ? 9101004 : 9001004).getEffect(1).applyTo(player); }
		 */
		/*
		 * if (MapConstants.isStorylineMap(c.getPlayer().getMapId())) { if (!c.getPlayer().hasSummon()) { c.getPlayer().setHasSummon(true); c.getSession().write(UIPacket.summonHelper(true)); c.getSession().write(UIPacket.summonMessage(TutorialConstants.getTutorialTalk(c.getPlayer(), c.getPlayer().getMapId()))); } }
		 */
