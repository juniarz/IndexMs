...
public class MapScriptMethods {
	...
	private static final String[] mulungEffects = { "I have been waiting for you! If you have an ounce of courage in you, you'll be walking in that door right now!", "How brave of you to take on Mu Lung Training Tower!", "I will make sure you will regret taking on Mu Lung Training Tower!", "I do like your intestinal fortitude! But don't confuse your courage with recklessness!", "If you want to step on the path to failure, by all means to do so!" };
	...
	public static void startScript_FirstUser(MapleClient c, String scriptName) {
		...
		switch (onFirstUserEnter.fromString(scriptName)) {/*
														 * case PTtutor000: c.getPlayer().getMap().startMapEffect("Please click on the npc to start your adventure", 5120016); break;
														 */
		case dojang_Eff: {
		...
		case shammos_Fenter: {
			if (c.getPlayer().getMapId() >= (GameConstants.GMS ? 921120100 : 921120005) && c.getPlayer().getMapId() < (GameConstants.GMS ? 921120300 : 921120500)) {
				...
				/*
				 * } else if (c.getPlayer().getMapId() == (GameConstants.GMS ? 921120300 : 921120500) && c.getPlayer().getMap().getAllMonstersThreadsafe().size() == 0) { final MapleMonster shammos = MapleLifeFactory.getMonster(9300281); if (c.getPlayer().getEventInstance() != null) { int averageLevel = 0, size = 0; for (MapleCharacter pl : c.getPlayer().getEventInstance().getPlayers()) { averageLevel += pl.getLevel(); size++; } if (size <= 0) { return; } averageLevel /= size; shammos.changeLevel(Math.max(120, Math.min(200, averageLevel))); } c.getPlayer().getMap().spawnMonsterOnGroundBelow(shammos, new Point(350, 170));
				 */
			}
			break;
		}
	...
	public static void startScript_User(final MapleClient c, String scriptName) {
		...
		/*
		 * case merTutorDrecotion00: { c.getSession().write(UIPacket.playMovie("Mercedes.avi", true)); final Map<Skill, SkillEntry> sa = new HashMap<>(); sa.put(SkillFactory.getSkill(20021181), new SkillEntry((byte) 1, (byte) 1, -1)); sa.put(SkillFactory.getSkill(20021166), new SkillEntry((byte) 1, (byte) 1, -1)); c.getPlayer().changeSkillsLevel(sa); break; }
		 */
		/*
		 * case merTutorDrecotion10: { c.getSession().write(UIPacket.getDirectionStatus(true)); c.getSession().write(UIPacket.IntroEnableUI(1)); c.getSession().write(UIPacket.getDirectionInfo("Effect/Direction5.img/effect/mercedesInIce/merBalloon/6", 2000, 0, -100, 1)); c.getSession().write(UIPacket.getDirectionInfo(1, 2000)); c.getPlayer().setDirection(0); break; }
		 */
		...
		/*
		 * final Map<Skill, SkillEntry> sa = new HashMap<>(); sa.put(SkillFactory.getSkill(20021181), new SkillEntry((byte) -1, (byte) 0, -1)); sa.put(SkillFactory.getSkill(20021166), new SkillEntry((byte) -1, (byte) 0, -1)); sa.put(SkillFactory.getSkill(20020109), new SkillEntry((byte) 1, (byte) 1, -1)); sa.put(SkillFactory.getSkill(20021110), new SkillEntry((byte) 1, (byte) 1, -1)); sa.put(SkillFactory.getSkill(20020111), new SkillEntry((byte) 1, (byte) 1, -1)); sa.put(SkillFactory.getSkill(20020112), new SkillEntry((byte) 1, (byte) 1, -1)); c.getPlayer().changeSkillsLevel(sa); c.getPlayer().changeJob((short)2300); MapleQuest.getInstance(24005).forceComplete(c.getPlayer(), 0); while (c.getPlayer().getLevel() < 10) { c.getPlayer().levelUp(); c.getPlayer().changeMap(101050010, 0); c.getSession().write(UIPacket.getDirectionStatus(false)); c.getSession().write(UIPacket.IntroEnableUI(0)); break; } }
		 */
		case ds_tuto_ani: {
		...
		/*
		 * case PTtutor100: { try { c.getSession().write(CField.UIPacket.IntroEnableUI(1)); c.getSession().write(CField.UIPacket.getDirectionInfoTest((byte)1, 8000)); c.getSession().write(CField.UIPacket.getDirectionInfoTest((byte)1, 8000)); c.getSession().write(CField.UIPacket.getDirectionInfoTest((byte)3, 2)); c.getPlayer().dropMessage(-1, "On A Rainy Day"); c.getPlayer().dropMessage(-1, "The Secret Garden Depths"); c.removeClickedNPC(); Thread.sleep(11000); } catch(InterruptedException e) {} c.getSession().write(CField.UIPacket.getDirectionStatus(false)); c.getSession().write(CField.UIPacket.IntroEnableUI(0)); c.removeClickedNPC(); c.getPlayer().dropMessage(-1, "click on ryden to get your first quest");
		 */
