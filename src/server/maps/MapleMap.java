...
public final class MapleMap {
	...
	/*
	 * Holds mappings of OID -> MapleMapObject separated by MapleMapObjectType. Please acquire the appropriate lock when reading and writing to the LinkedHashMaps. The MapObjectType Maps themselves do not need to synchronized in any way since they should never be modified.
	 */
	...
	private int consumeItemCoolTime = 0, protectItem = 0, decHPInterval = 10000, mapid, returnMapId, timeLimit, fieldLimit, maxRegularSpawn = 0, fixedMob, forcedReturnMap = 999999999, instanceid = -1, lvForceMove = 0, lvLimit = 0, permanentWeather = 0, partyBonusRate = 0;
	private boolean town, clock, personalShop, everlast = false, dropsDisabled = false, gDropsDisabled = false, soaring = false, squadTimer = false, isSpawns = true, checkStates = true;
	...
	/*
	 * command to reset all item-reactors in a map to state 0 for GM/NPC use - not tested (broken reactors get removed from mapobjects when destroyed) Should create instances for multiple copies of non-respawning reactors...
	 */
	...
	/*
	 * public final void spawnMonster(final MapleMonster monster, final int spawnType) { spawnMonster(monster, spawnType, false); }
	 * 
	 * public final void spawnMonster(final MapleMonster monster, final int spawnType, final boolean overwrite) { monster.setMap(this); checkRemoveAfter(monster);
	 * 
	 * spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
	 * 
	 * public final void sendPackets(MapleClient c) { c.getSession().write(MobPacket.spawnMonster(monster, monster.getStats().getSummonType() <= 1 || monster.getStats().getSummonType() == 27 || overwrite ? spawnType : monster.getStats().getSummonType(), 0)); } }); updateMonsterController(monster);
	 * 
	 * spawnedMonstersOnMap.incrementAndGet(); }
	 */
	...
	public final void addPlayer(final MapleCharacter chr) {
		...
		/*
		 * if (mapid == 0) { if (GameConstants.isBeginnerJob(chr.getJob())) { NPCScriptManager.getInstance().start(chr.getClient(), TutorialConstants.beginnerNPC); } } else if (MapConstants.isStorylineMap(mapid)) { chr.getMap().startSimpleMapEffect(TutorialConstants.getStageMSG(chr, mapid), 5120018); if (!chr.hasSummon()) { chr.setHasSummon(true); chr.getClient().getSession().write(UIPacket.summonHelper(true)); chr.getClient().getSession().write(UIPacket.summonMessage(TutorialConstants.getTutorialTalk(chr, mapid))); } else { chr.getClient().getSession().write(UIPacket.summonMessage(TutorialConstants.getTutorialTalk(chr, mapid))); } }
		 */
		/*
		 * if (mapid == 552000010) { chr.getClient().getSession().write(CField.UIPacket.IntroEnableUI(1)); chr.getClient().getSession().write(CField.UIPacket.getDirectionInfoTest((byte)1, 1000)); chr.getClient().getSession().write(CField.UIPacket.getDirectionInfoTest((byte)1, 3000)); chr.getClient().getSession().write(CField.UIPacket.getDirectionInfoTest((byte)3, 1)); chr.getClient().removeClickedNPC(); NPCScriptManager.getInstance().start(chr.getClient(), 9250135); }
		 */
		...
		/*
		 * if (mapid == 915000500 ) { chr.getClient().getSession().write(UIPacket.playMovie("phantom_memory.avi", true)); }
		 */
		if (mapid == 103050900) {
			try {
				chr.getClient().getSession().write(CField.UIPacket.IntroEnableUI(1));
		...
		/*
		 * if (mapid == 913070000) { try { chr.getClient().getSession().write(CField.UIPacket.IntroEnableUI(1)); chr.dropMessage(-1, "Month 3, Day 4"); chr.dropMessage(-1, "Mr.Limbert's General Store");
		 * 
		 * chr.getClient().getSession().write(UIPacket.getDirectionInfo("Effect/Direction4.img/effect/tuto/step0/0", 5000, 0, 0, 1)); chr.getClient().getSession().write(UIPacket.getDirectionInfo("Effect/Direction4.img/effect/tuto/step0/1", 5000, 0, 0, 1)); chr.getClient().getSession().write(UIPacket.getDirectionInfo("Effect/Direction4.img/effect/tuto/step0/2", 5000, 0, 0, 1)); chr.getClient().getSession().write(UIPacket.getDirectionInfo("Effect/Direction4.img/effect/tuto/step0/3", 5000, 0, 0, 1)); chr.getClient().getSession().write(CField.UIPacket.getDirectionInfoTest((byte)1, 8000)); chr.getClient().getSession().write(CField.UIPacket.getDirectionInfoTest((byte)1, 8000)); chr.getClient().getSession().write(CField.UIPacket.getDirectionInfoTest((byte)3, 2)); chr.getClient().removeClickedNPC(); Thread.sleep(11000); } catch(InterruptedException e) {} chr.getClient().getSession().write(CField.UIPacket.getDirectionStatus(false)); chr.getClient().getSession().write(CField.UIPacket.IntroEnableUI(0)); chr.getClient().removeClickedNPC(); }
		 */

		/*
		 * c.getSession().write(UIPacket.IntroDisableUI(true)); c.getSession().write(UIPacket.IntroLock(true)); c.getSession().write(UIPacket.getDirectionInfo("Effect/Direction4.img/effect/tuto/step0/0", 5000, 0, 0, 1)); c.getSession().write(UIPacket.getDirectionInfo("Effect/Direction4.img/effect/tuto/step0/1", 5000, 0, 0, 1)); c.getSession().write(UIPacket.getDirectionInfo("Effect/Direction4.img/effect/tuto/step0/2", 5000, 0, 0, 1)); c.getSession().write(UIPacket.getDirectionInfo("Effect/Direction4.img/effect/tuto/step0/3", 5000, 0, 0, 1)); c.getSession().write(EffectPacket.ShowWZEffect("Effect/Direction4.img/cannonshooter/face04")); c.getSession().write(EffectPacket.ShowWZEffect("Effect/Direction4.img/cannonshooter/out01")); c.getSession().write(UIPacket.getDirectionInfo(1, 5000)); }
		 */
	...
	/*
	 * public void broadcastMessage(MapleCharacter source, byte[] packet, boolean repeatToSource, boolean ranged) { broadcastMessage(repeatToSource ? null : source, packet, ranged ? MapleCharacter.MAX_VIEW_RANGE_SQ : Double.POSITIVE_INFINITY, source.getPosition()); }
	 */
